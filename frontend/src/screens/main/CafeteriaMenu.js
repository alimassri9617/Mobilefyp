...
export default function CafeteriaMenu() {
  ...
  const handleSave = async () => {
    const { _id, name, protein, calories, image, category } = editingItem;
    try {
      await fetch(`http://localhost:5000/api/menu/${selectedDay}/${category}/${_id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${authUser?.token}`,
  ...
  const handleDelete = async (itemId, category) => {
    try {
      await fetch(`http://localhost:5000/api/menu/${selectedDay}/${category}/${itemId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${authUser?.token}`,
        },
  ...
  const handleAddItem = async () => {
    ...
    try {
      await fetch(`http://localhost:5000/api/menu/${selectedDay}/${category}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${authUser?.token}`,
